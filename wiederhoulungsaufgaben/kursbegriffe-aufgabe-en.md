...
The first section of a JSON Web Token containing metadata about the token's type and the signing algorithm used (e.g., {"alg": "HS256", "typ": "JWT"}).

- JWT payload

The middle section of a JSON Web Token containing claims—statements about the user (e.g., user ID, roles) and token metadata.

- Claim

An individual key-value data pair stored inside a JWT payload (like expiration time or user permissions).

- JWT signature

The cryptographic string generated by combining the encoded header, payload, and a secret/private key, used to verify token authenticity and detect tampering.

- Base64URL

An encoding algorithm derived from Base64 that safely encodes binary data into a URL-friendly text format by omitting problematic characters like +, /, and =.

- JWS

The official standard specifying how to digitally sign content (like a JWT) to ensure data integrity and origin authenticity.

- `HS256`

A symmetric HMAC cryptographic algorithm that uses a single shared secret key to both create and verify JWT signatures.

- `RS256`

An asymmetric RSA algorithm that uses a private key to sign a JWT and a public key to verify it.

- `sub` -> subject

A standard JWT claim indicating the principal subject of the token (typically the unique user ID).

- `iss` -> issuer

A standard JWT claim identifying the domain or authorization server that created and issued the token.

- `aud` -> audience

A standard JWT claim identifying the target recipients or API services for which the token is intended.

- `exp` -> expiration time

A standard JWT claim containing a timestamp defining when the token automatically expires and becomes invalid.

- `nbf` -> not before

A standard JWT claim specifying the exact timestamp before which the token must not be accepted for processing.

- `iat` -> issued at

A standard JWT claim identifying the exact timestamp at which the token was created.

- `jti` -> JWT ID

A standard JWT claim providing a unique identifier for the specific token, often used to prevent token replay attacks or manage revocation lists.

- `jsonwebtoken`

The standard npm library used in Node.js applications to sign, verify, and decode JSON Web Tokens.

- `jwt.sign()`

The method in the jsonwebtoken library used to generate a new signed JWT with a given payload and secret key.

- `jwt.verify()`

The method in the jsonwebtoken library used to cryptographically check a token's signature, validate claims, and return the decoded payload.

- `jwt.decode()`

A method used to read and view a JWT's decoded payload content without checking or verifying its cryptographic signature (unsafe for authorization!).

- Refresh token

A long-lived credential used securely behind the scenes to request and obtain new short-lived access tokens without forcing the user to log in again.

- Token rotation

A security mechanism where issuing a new access token also invalidates the old refresh token and issues a new one, mitigating refresh token theft.

- Token revocation

The process of explicitly invalidating an active token before its natural expiration date (usually managed via deny-lists or database lookups).

## API Security and CORS

- Attack surface

The total sum of all possible entry points, endpoints, and interfaces through which an unauthorized user or hacker could attempt to exploit a system.

- Input validation

The practice of checking incoming request data against defined rules (types, formats, ranges) to reject invalid data before processing.

- Parsing

The process of analyzing a raw data string (like raw JSON or query parameters) and converting it into a usable data object in memory.

- Normalization

The practice of sanitizing and standardizing incoming data into a uniform format (e.g., converting emails to lowercase) before processing or saving.

- Allowlist / whitelist

An explicit security model that denies all access or inputs by default and only permits specifically specified items or origins.

- Denylist / blacklist

A security model that allows all access or inputs by default except those explicitly marked as forbidden or dangerous (generally less secure than allowlisting).

- Injection

A major vulnerability class where untrusted input is executed as code by an interpreter due to a lack of proper sanitization or parameterization.

- SQL injection

A critical security vulnerability where an attacker injects malicious SQL fragments into input fields to manipulate database queries.

- NoSQL injection

An attack targeting NoSQL databases (like MongoDB) where malicious input objects override query filters to bypass authentication or leak data.

- Command injection

A severe vulnerability where unvalidated user input is executed directly as system shell commands on the server host.

- Broken Object Level Authorization

An API flaw where an application fails to check if the authenticated user has permission to access a specific resource ID (e.g., viewing another user's invoice by changing /api/invoices/10 to /11).

- Broken Object Property Level Authorization

An API vulnerability where users can view or modify sensitive object fields (like isAdmin or balance) they shouldn't have access to.

- Mass assignment

A flaw where an application automatically maps raw request body fields directly to internal model properties, allowing users to overwrite protected fields.

- Data minimization

The practice of limiting data collection and API responses to only the strict minimum necessary for the immediate business function.

- Rate limiting

A defensive strategy that caps the number of requests a client IP or account can make to an API within a given timeframe to prevent abuse.

- Unrestricted resource consumption

A security flaw where an API lacks bounds on resource usage (like page sizes or file uploads), exposing it to Denial of Service (DoS) attacks.

- Security misconfiguration

Security flaws resulting from default settings, incomplete setup, unpatched software, or exposed verbose error logs on web servers.

- Same-origin principle

A fundamental browser security rule that restricts scripts on one domain from reading data from another domain unless permission is explicitly given.

- Origin

The combination of scheme (protocol), host (domain), and port number used to uniquely identify the source of a web request.

- Cross-origin request

An HTTP request made by client code running on one origin to a completely different domain, sub-domain, protocol, or port.
...
An automatic OPTIONS HTTP request sent by browsers before a cross-origin request to ask the server if the intended main request is safe to send.
...
A CORS header sent by the server indicating which HTTP methods (e.g., GET, POST, DELETE) are allowed during cross-origin requests.

- `Access-Control-Allow-Headers`

A CORS header indicating which custom HTTP headers can be sent during actual cross-origin requests.

- HTTPS/TLS

Protocols used to encrypt communication between web browsers and servers, ensuring data privacy, integrity, and protection against tampering.

- Logging

The systematic recording of application events, server requests, errors, and system status into log files for debugging and auditing.

- Monitoring

The continuous process of tracking backend health, application metrics, server traffic, and system errors in real time.

- Deny by default

A foundational security design principle stating that access rights should be blocked unless explicitly granted to an entity.

- Never trust the client

The golden rule of backend development stating that all client data (inputs, headers, cookies, parameters) must be treated as untrusted and validated on the server.

## Other abbreviations or terms I stumble uppon

#### Metadata

Is "data about data." In the context of JWT, instead of being the main content of a message (like the actual JSON user object), metadata provides context about the message—such as how big the payload is, what format it is written in (Content-Type: application/json), or who sent it (Authorization: Bearer token).
